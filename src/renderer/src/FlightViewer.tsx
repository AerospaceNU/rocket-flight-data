...
export function FlightViewer({
  ...
  useEffect(() => {
    ...
    window.appBridge
      ...
      .then((attrs) => {
        if (ignore) return;
        // Only the manual shared keys are editable here; autogenerated rollups
        // (peaks) and the folder-derived date/name are preserved server-side.
        const editable = FLIGHT_REQUIRED_ATTRIBUTE_KEYS.map((key) => ({
          key,
          value: attrs.find((attribute) => attribute.key === key)?.value ?? ''
        }));
      ...
      .catch(() => {
        if (ignore) return;
        const editable = FLIGHT_REQUIRED_ATTRIBUTE_KEYS.map((key) => ({ key, value: '' }));
        setFlightAttributes(editable);
        setLoadedFlightAttributes(editable);
      });
