...
export async function saveFlightAttributes(
  ...
  resolveInsideOutputDirectory(outputDirectory, path.join(outputDirectory, flightDirectoryName));
  // Merge the submitted keys over the existing flight file so a focused editor
  // (motor, location) cannot drop autogenerated rollups (peaks) or date/name.
  const existing = await readFlightAttributesRecord(outputDirectory, flightDirectoryName);
  const merged: Record<string, string> = { ...existing };
  for (const attribute of attributes) {
    const key = attribute.key.trim();
    if (key) merged[key] = attribute.value;
  }
  await writeFlightAttributesRecord(outputDirectory, flightDirectoryName, merged);
